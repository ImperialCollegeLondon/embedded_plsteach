...
def create_app(test_config=None):
    ...
    @app.route('/') #handle for index request
    def index():
        if g.user is None:
            return render_template('index.html')
        else:
            return redirect(url_for('main.home')) #redirect to main page if logged in
    ...
    #import Connections.py and instantiate socketio namespace object
    from . import Connections
    socketio.init_app(app)
    socketio.on_namespace(Connections.Connections(10, '/'))
    
    mqtt.init_app(app) #initialize  mqtt
    return app
